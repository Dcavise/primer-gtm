-- Modify the function to use case-insensitive matching for campus names
...
BEGIN
  ...
  ELSIF campus_name IS NOT NULL THEN
    -- Filter by specific campus using case-insensitive matching
    -- This handles capitalization differences and trims whitespace
    campus_filter := format(' AND (TRIM(LOWER(l.preferred_campus_c)) = LOWER(%L))', TRIM(campus_name));
  ELSE
...
COMMENT ON FUNCTION fivetran_views.get_lead_metrics IS 'Counts distinct leads directly from lead table, grouped by day/week/month and campus, with case-insensitive filtering options';
