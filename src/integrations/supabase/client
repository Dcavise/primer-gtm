...
class SupabaseUnifiedClient {
    ...
    try {
      ...
      if (this.hasAdminAccess()) {
        ...
        if (!adminError && adminData) {
          return { success: true, data: adminData, error: null, usingAdminClient: true };
        }
      ...
      return { 
        success: false, 
        data: null, 
        error: regularError || new Error('Failed to query Salesforce table and admin client not available')
      };
    } catch (error) {
  ...
  public async executeRPC(functionName: string, params: Record<string, any> = {}) {
    try {
      ...
      const { data: regularData, error: regularError } = await this.regular.rpc(functionName, params);
      ...
      return { 
        success: false, 
        data: null, 
        error: regularError || new Error(`Failed to execute RPC ${functionName} and admin client not available`)
      };
    } catch (error) {
      ...
      try {
        ...
        if (this.hasAdminAccess()) {
          try {
            ...
            fivetranAccess = !result.error && result.data && result.data[0] && result.data[0].exists;
