 ...
 */
export const useDashboardCharts = (selectedCampusIds: string[], selectedCampusNames: string[]) => {
  ...
  const opportunityStagesQuery = useQuery({
    ...
    queryFn: async (): Promise<OpportunityStageData[]> => {
      const { data, error } = await supabase.rpc("get_opportunities_by_stage_campus");
      ...
      if (selectedCampusIds.length > 0) {
        processedData = data.filter((item: any) => selectedCampusIds.includes(item.campus_id));
      }
  ...
  const isLoading = weeklyLeadsQuery.isLoading || opportunityStagesQuery.isLoading;
