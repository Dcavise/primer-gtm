...
export function useFellowsStats(selectedCampusIds: string[]): FellowsStats {
  ...
  const fetchFellowsStats = useCallback(async () => {
    logger.info(`Fetching fellows stats for campuses: ${selectedCampusIds.length > 0 ? selectedCampusIds.join(', ') : "all campuses"}`);

    // Query fellows data
    const fellowsData = await queryTable<any[]>(
      'fellows',
      (query) => {
    ...
    if (fellowsData && Array.isArray(fellowsData)) {
      ...
      if (fellowsData.length > 0) {
        ...
        return { fellowsCount: fellowsData.length, employmentStatusCounts: statusCountsArray };
      }
    }
