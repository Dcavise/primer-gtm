...
export const MetricsCharts: React.FC<MetricsChartsProps> = memo(({ 
  timeSeriesData,
  period
}) => {
  ...
  const formatXAxis = useCallback((value: string): string => {
  ...
  const combinedData = useMemo<ChartDataPoint[]>(() => {
    ...
    if (displayData[0]?.data) {
      for (let i = 0; i < displayData[0].data.length; i++) {
        const dataPoint: ChartDataPoint = { date: displayData[0].data[i].date };
