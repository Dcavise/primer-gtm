...
import { Building, MapPin, Calendar, CalendarDays, CircleCheck, Clock, ArrowRight, Plus, ChevronDown } from 'lucide-react';
...
const STAGE_COLORS = {
  'Diligence': { bg: 'bg-blue-100', text: 'text-blue-600', border: 'border-blue-200', add: 'text-blue-500' },
  'LOI': { bg: 'bg-amber-100', text: 'text-amber-600', border: 'border-amber-200', add: 'text-amber-500' },
  'Lease': { bg: 'bg-green-100', text: 'text-green-600', border: 'border-green-200', add: 'text-green-500' },
  'Build Out': { bg: 'bg-purple-100', text: 'text-purple-600', border: 'border-purple-200', add: 'text-purple-500' }
};

// Sample property addresses for the new design with state and city information
const SAMPLE_ADDRESSES = {
  'Diligence': [
    { address: '6007 111th Street East, Bradenton, FL 34211', status: 'Site assessment', state: 'Florida', city: 'Bradenton' },
    { address: '4400 Mobile Hwy, Pensacola, FL 32609', status: 'Financial modeling', state: 'Florida', city: 'Pensacola' },
    { address: '2301 McFarland Blvd E, Tuscaloosa, AL 35404', status: 'Initial review', state: 'Alabama', city: 'Tuscaloosa' },
    { address: '7900 Eastern Blvd, Montgomery, AL 36117', status: 'Financial modeling', state: 'Alabama', city: 'Montgomery' },
    { address: '2122 E Rio Salado Pkwy, Tempe, AZ 85281', status: 'Site assessment', state: 'Arizona', city: 'Tempe' },
    { address: '7000 E Mayo Blvd, Phoenix, AZ 85054', status: 'Initial review', state: 'Arizona', city: 'Phoenix' }
  ],
  'LOI': [],
  'Lease': [],
  'Build Out': []
...
const mapPhaseToStage = (phase: PropertyPhase): string => {
  if (phase.startsWith('1.') || phase.startsWith('2.') || phase.startsWith('3.')) return 'Diligence';
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="relative">
                  <div className="flex items-center relative">
                    <label htmlFor="state-selector" className="absolute left-3 text-sm font-medium text-slate-gray sr-only">State:</label>
                    <select 
                      id="state-selector"
                ...
                {selectedState !== 'All States' && (
                  <div className="relative">
                    <div className="flex items-center relative">
                      <label htmlFor="city-selector" className="absolute left-3 text-sm font-medium text-slate-gray sr-only">City:</label>
                      <select 
                        id="city-selector"
                        className="h-10 w-full min-w-[160px] border border-platinum rounded-md pl-4 pr-8 py-2 shadow-sm hover:border-french-gray focus:outline-none bg-white text-sm appearance-none"
                        value={selectedCity}
                ...
                <Button variant="outline" size="default" className="gap-1.5 border-platinum bg-white shadow-sm hover:bg-platinum hover:text-outer-space">
          ...
          {STAGES.map(stage => {
            ...
            return (
              <div key={stage} className="flex flex-col">
                ...
                <div className={`mb-3 px-3 py-1.5 flex items-center justify-between ${stageColors.bg} rounded-md`}>
                  <h3 className={`text-sm font-medium ${stageColors.text}`}>{stage}</h3>
                  <div className="flex items-center">
                    <span className={`text-xs ${stageColors.text}`}>{filteredProperties[stage]?.length || 0}</span>
                  </div>
                </div>
