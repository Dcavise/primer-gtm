...
import { ExternalLink, Calendar, Info, MapPin, User, Building, FileText, Clock } from "lucide-react";
...
export const PermitDetail = ({ permit }: PermitDetailProps) => {
  ...
  return (
    <div className="animate-fade-in px-1">
      ...
      <DetailSection title="Project Information" icon={<Info className="h-5 w-5 text-zoneomics-blue" />}>
        ...
        <div className="space-y-1">
          ...
          {permit.contact_phone_number && (
            <DetailItem label="Phone" value={
              <a href={`tel:${permit.contact_phone_number}`} className="text-primary hover:underline">
                {permit.contact_phone_number}
              </a>
            } />
          )}
      ...
      <DetailSection title="Dates" icon={<Calendar className="h-5 w-5 text-zoneomics-blue" />}>
        <div className="space-y-1">
          <DetailItem label="Filed Date" value={permit.date ? formatDate(permit.date) : "Unknown"} />
          <DetailItem label="Created" value={permit.created_date ? formatDate(permit.created_date) : "Unknown"} />
          <DetailItem label="Last Updated" value={permit.last_updated_date ? formatDate(permit.last_updated_date) : "Unknown"} />
        </div>
      </DetailSection>

      {(permit.record_link || permit.document_link) && (
        <DetailSection title="Documents" icon={<FileText className="h-5 w-5 text-zoneomics-blue" />}>
      ...
      {permit.comments && (
        <DetailSection title="Additional Information" icon={<Info className="h-5 w-5 text-zoneomics-blue" />}>
